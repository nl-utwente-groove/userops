...
\paragraph{\UserType.} This is a class-level annotation, to be used on public \recordT types with all field types taken from the \javaA algebra (\boolT, \intT, \doubleT or \stringT).
%
\begin{lstlisting}
@UserType
...
\end{lstlisting}
%
defines an operator $\nm\in O^\userS$ with $\sigma_\nm=s_1\cdots s_n$ (where the $s_i$ are the sorts corresponding to the Java types $\ty_i$) as well as operators $\fd_i\in O^{s_i}$ with $\sigma_{\fd_i}=\userS$ for all $1\leq i\leq n$, together with corresponding functions $f_\nm$, $f_{\fd_1},\ldots, f_{\fd_n}$. In terms of \Cref{sec:user}, $\nm$ is a constructor and the $\fd_i$ are accessors.

\paragraph{\UserOperation.} This is a method-level annotation to define ``other operators,'' to be used on public methods in arbitrary public classes. All parameter types as well as the return type are either taken from the \javaA algebra, \emph{or} may be \UserType-annotated records. If the containing class is not a \UserType, the annotated method must be \staticT. Hence there are two cases.
...
\monthO      & \userS                & \intS  \\
\yearO       & \userS                & \intS  \\
\end{array}\qquad\qquad\begin{array}[t]{l@{{\quad}}c@{{\quad}}c@{{\quad}}c}
o            & \sigma_o & s_o \\
\hline
...
\vspace*{-5mm}
\end{wrapfigure}
To actually \emph{use} annotation-defined operators, a rule system has to specify the qualified names of the annotated classes in its System Properties. From then on, they can be treated like built-in system operators.

\Cref{fig:rules} shows two example \GROOVE rules using the operators of \Cref{fig:Date}.
%
\begin{figure}
\subcaptionbox{\createR: a \DateL-typed node is transformed to a \PersonL}[.6\textwidth]{\includegraphics[scale=.17]{figs/create}}
\subcaptionbox{\birthR: a \pregnantL \PersonL gives birth}[.4\textwidth]{\includegraphics[scale=.17]{figs/birth}}
...
%
\begin{figure}[t]%
\subcaptionbox{Start graph}[.2\textwidth]{\includegraphics[scale=.17]{figs/start}}%
\subcaptionbox{Transition system}[.55\textwidth]{\includegraphics[scale=.14]{figs/user-gts}}%
\subcaptionbox{Final graph}[.25\textwidth]{\includegraphics[scale=.17]{figs/final}}
